...
import { Building2, FileText, ShoppingCart, Home } from 'lucide-react';
        ...
        <div className="bg-neutral-900 p-5 md:p-6 rounded-2xl border border-neutral-800 mb-6">
          ...
          <div className="space-y-3 mb-4">
            <div className="flex justify-between items-center text-sm border-b border-neutral-800/50 pb-2">
              ...
              <span className="text-orange-400 font-medium">+ R$ <NumberTicker value={results.companyCostBreakdown.inssPatronal.value + results.companyCostBreakdown.sistemaS.value + results.companyCostBreakdown.satRat.value + results.companyCostBreakdown.fgts.value + results.companyCostBreakdown.fgtsProvisoes.value} /></span>
            </div>
            <div className="flex justify-between items-center text-sm border-b border-neutral-800/50 pb-2">
              <span className="text-neutral-300 flex items-center gap-2">
                <FileText size={16} className="text-neutral-400" /> Retido na Fonte (Seu INSS + IRPF)
              </span>
              <span className="text-red-400 font-medium">+ R$ <NumberTicker value={results.employeeCostBreakdown.inss.value + results.employeeCostBreakdown.irpf.value} /></span>
            </div>
            <div className="flex justify-between items-center text-sm border-b border-neutral-800/50 pb-2">
              <span className="text-neutral-300 flex items-center gap-2">
                <ShoppingCart size={16} className="text-neutral-400" /> Impostos de Consumo (Embutido nas compras)
              </span>
              <span className="text-red-400 font-medium">+ R$ <NumberTicker value={results.monthlyIndirectTaxes} /></span>
            </div>
            {(results.yearlyIpva > 0 || results.yearlyIptu > 0) && (
              <div className="flex justify-between items-center text-sm border-b border-neutral-800/50 pb-2">
                ...
                <span className="text-red-400 font-medium">+ R$ <NumberTicker value={(results.yearlyIpva + results.yearlyIptu) / 12} /></span>
              </div>
            )}
          </div>
